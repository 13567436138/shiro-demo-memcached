...
import com.mark.demo.shiro_memched.constant.PageConst;
...
public class PaginateUtils
{
    ...
    /**
     * 根据Pagination对象取得完整的Pagination对象(当每次查询列表之前用count查�?�记录数时用该方�?)
    ...
    /**
     * 根据每页笔数、当前页数�?�记录�?�数取得完整的Pagination对象(缓存count时用该方�?)
     * @param pageSize 每页显示的资料笔�?
    ...
    /**
     * 根据当前页标取得是否有上�?页的标记(根据此方法设定上�?�?,首页是否有超链接)
     * @param currentPage 当前显示的页�?
     * @return boolean 是否有上�?页的标记(true：有,false：无)
     */
    ...
    /**
     * 根据当前显示的页数及资料总页数判断是否该有下�?�?
     * @param currentPage 当前显示的页�?
     * @param totalPage 资料总页�?
     * @return boolean 否该有下�?�?(true：有,false：无)
     */
