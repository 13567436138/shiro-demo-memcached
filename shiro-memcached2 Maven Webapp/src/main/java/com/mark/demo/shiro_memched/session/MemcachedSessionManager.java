package com.mark.demo.shiro_memched.session;
...
import com.mark.demo.shiro_memched.filter.SessionFilter;
import com.mark.demo.shiro_memched.utils.CookieUtils;
import com.mark.demo.shiro_memched.utils.MemcachedUtils;

public class MemcachedSessionManager
{
    public static final Logger   logger               = LoggerFactory.getLogger(MemcachedSessionManager.class);
    ...
    public boolean put(HttpServletRequest request, SessionKey key, Object value, int expireSecond)
    {
        ...
        if (StringUtils.isNotBlank(realKey))
        {
        	...
        	MemcachedUtils.set(realKey, value, expireSecond);
            return true;
        }
        return false;
    }
    ...
    public Object getObject(HttpServletRequest request, SessionKey key)
    {
        ...
        if (StringUtils.isNotBlank(realKey)) { 
        	...
        	return MemcachedUtils.get(realKey);
        }
        return null;
    }
    ...
    public void expire(HttpServletRequest request, SessionKey key)
    {
        ...
        if (StringUtils.isNotBlank(realKey))
        {
        	...
        	MemcachedUtils.expire(realKey, defaultExpireSeconds);
        }
    }
